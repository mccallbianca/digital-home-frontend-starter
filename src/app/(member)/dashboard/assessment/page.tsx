    ...
    <main className="min-h-screen">
      <section className="px-6 pt-32 pb-16 border-b border-[rgba(26,15,26,0.08)]" style={{ background: 'var(--herr-cream)' }}>
        <div className="max-w-[900px] mx-auto">
          ...
          <p className="herr-label text-[var(--herr-magenta)] mb-4">My Progress</p>
      ...
      <section className="px-6 py-16" style={{ background: 'var(--herr-cream)' }}>
        <div className="max-w-[900px] mx-auto">
          ...
          {latestReport && (
            <div className="mb-16">
              <p className="herr-label text-[var(--herr-magenta)] mb-6">Progress Comparison</p>
              <ProgressChart
          ...
          {hasResponses ? (
                ...
                {DOMAINS.map(domain => {
                  ...
                  return (
                    <div key={domain.label} className="p-6 rounded" style={{ background: '#FFFFFF', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                      <p className="font-medium text-sm mb-3" style={{ color: 'var(--herr-ink)' }}>{domain.label}</p>
                      <div className="w-full h-2 overflow-hidden mb-2" style={{ background: '#E8E4DE', borderRadius: '4px' }}>
                        <div
                          className="h-full bg-[var(--herr-magenta)] transition-all duration-700"
              ...
              <div className="space-y-6">
                {QUESTIONS.map((q, idx) => (
                  <div key={idx} className="pb-6" style={{ borderBottom: '1px solid #E8E4DE' }}>
                    <p className="mb-2 leading-relaxed" style={{ color: 'var(--herr-ink)', fontSize: '1rem' }}>
                      <span className="text-[var(--herr-magenta)] font-display mr-2">{idx + 1}.</span>
                      {q}
                    </p>
                    <p className="text-sm" style={{ color: '#666666' }}>
                      Your response: <span style={{ color: 'var(--herr-ink)' }}>{LIKERT_LABELS[responseMap[idx] ?? 0] || '—'}</span>
                    </p>
                  </div>
                ))}
              </div>
            ...
            <div className="p-8 rounded" style={{ background: '#FFFFFF', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', borderLeft: '3px solid var(--herr-magenta)' }}>
              <p className="herr-label text-[var(--herr-magenta)] mb-3">Ready for a Fresh Assessment</p>
              ...
              <div className="space-y-3">
                ...
                {snapshots.map((snap: any, i: number) => (
                  <div key={i} className="px-6 py-4 flex items-center justify-between rounded" style={{ background: '#FFFFFF', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                    <p className="text-sm" style={{ color: 'var(--herr-ink)' }}>
