...
export default async function AffirmationsPage() {
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'var(--herr-cream)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '80px 24px' }}>
        <div style={{ maxWidth: 480, textAlign: 'center' }}>
          ...
          <h1 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 24, color: 'var(--herr-ink)', marginBottom: 8 }}>Unlock Daily Affirmations</h1>
          <p style={{ fontSize: 15, color: 'rgba(26,15,26,0.65)', lineHeight: 1.6, marginBottom: 24 }}>
            Upgrade to HERR Collective to receive daily affirmations in Bianca&apos;s voice,
            or HERR Personalized to hear them in your own.
          </p>
          <a href="/checkout" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 48, padding: '0 32px', background: 'var(--herr-magenta)', color: 'var(--herr-cream)', borderRadius: 12, fontSize: 14, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px', textDecoration: 'none' }}>
            Upgrade Now
          </a>
        </div>
      </div>
  ...
  // Fetch affirmation tracks (excluding soft-deleted)
  ...
  // For Personalized/Elite: fetch latest affirmation + ECQO Sound catalog tracks for inline layered player
