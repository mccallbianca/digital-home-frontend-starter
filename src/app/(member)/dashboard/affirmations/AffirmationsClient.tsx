...
export default function AffirmationsClient({
  ...
  async function handleGenerateToday() {
    ...
    try {
      const todayMode = selectedModes[0] || 'morning';
      const res = await fetch('/api/affirmations/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId, activityMode: todayMode }),
      });
      if (res.ok) {
        // Reload so the server fetches the new row.
        window.location.reload();
      } else {
        const body = await res.json().catch(() => ({}));
        setGenerationError(body.error || `Generation failed (HTTP ${res.status})`);
      }
    } catch (err) {
    ...
    return (
      <div key={track.id} style={CARD_BASE}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--herr-ink-soft)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ width: '100%', height: 4, background: 'var(--herr-line)', borderRadius: 2, overflow: 'hidden' }}>
              <div
                style={{
                  height: '100%',
          ...
          <span style={{ fontSize: 12, color: 'var(--herr-ink-soft)', flexShrink: 0 }}>
  ...
  // Empty-today state with explicit retry — replaces the silent dev-overlay error symptom.
  const renderTodayEmpty = () => (
    <div style={{ ...CARD_BASE, textAlign: 'center', padding: 36 }}>
      <p style={{ fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.12em', color: 'var(--herr-magenta)', fontWeight: 600, marginBottom: 12 }}>
        Today
      </p>
      <h3 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 22, fontWeight: 500, color: 'var(--herr-ink)', marginBottom: 10 }}>
        We&apos;re generating today&apos;s affirmation now.
      </h3>
      <p style={{ fontSize: 14, color: 'var(--herr-ink-soft)', lineHeight: 1.6, marginBottom: 20, maxWidth: 440, marginLeft: 'auto', marginRight: 'auto' }}>
        Daily delivery runs at 4 AM ET. If you don&apos;t see it yet, you can trigger
        it manually below.
      </p>
      {generationError && (
        <p style={{ fontSize: 13, color: 'var(--herr-magenta-deep)', marginBottom: 12 }}>
          {generationError}
        </p>
      )}
      <button
        ...
        style={{
          ...
          gap: 8,
          padding: '12px 24px',
          background: generating ? 'var(--herr-ink-soft)' : 'var(--herr-magenta)',
  ...
  // Personalized + Elite tier: embedded layered player (full 3-stem mixing UI, v3 gain spec).
  const renderLayeredSection = () => (
    ...
    >
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--herr-ink-soft)', lineHeight: 1.5, marginTop: 6 }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--herr-cream)', padding: '40px 24px 80px' }}>
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        ...
        <h1 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 34, fontWeight: 500, color: 'var(--herr-ink)', marginBottom: 4 }}>
        ...
        {archiveTracks.length > 0 && (
          <div style={{ borderTop: '1px solid var(--herr-line)', paddingTop: 32, marginTop: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <p style={{ fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.18em', color: 'var(--herr-ink-soft)', fontWeight: 600 }}>
                ARCHIVE
              </p>
              <select
                value={filter}
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      <span style={{ fontSize: 13, color: 'var(--herr-ink-soft)' }}>{date}</span>
                      <span
                        style={{
                          fontSize: 10,
                          color: 'var(--herr-magenta)',
                    ...
                    <p style={{ fontSize: 14, color: 'var(--herr-ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', margin: 0 }}>
                      {excerpt}
                    </p>
                  </div>
                  <button
