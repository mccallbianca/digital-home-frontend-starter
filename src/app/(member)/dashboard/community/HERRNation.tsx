...
export default function HERRNation({ userId, displayName: _displayName, userTier, acknowledged }: HERRNationProps) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-24">
        <div className="w-8 h-8 border-2 border-[var(--herr-magenta)] border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }
  ...
  /* ======================= RENDER ======================= */
  return (
    <div className="flex h-[calc(100vh-280px)] min-h-[500px] border border-[rgba(26,15,26,0.08)]">
      {/* ---- Sidebar ---- */}
      <aside className={`
        ${sidebarOpen ? 'block' : 'hidden'} md:block
        w-full md:w-56 shrink-0 border-r border-[rgba(26,15,26,0.08)] bg-[var(--herr-cream)]
        absolute md:relative z-10 md:z-auto h-full overflow-y-auto
      `}>
        <div className="p-4">
          <p className="herr-label text-[rgba(26,15,26,0.4)] mb-3">Spaces</p>
          {spaces.map(s => (
            <button
              key={s.slug}
              ...
              className={`block w-full text-left px-3 py-2 text-[0.85rem] rounded transition-colors mb-1 ${
                activeSpace === s.slug && view === 'spaces'
                  ? 'bg-[#FFFFFF] text-[var(--herr-ink)]'
                  : 'text-[rgba(26,15,26,0.6)] hover:text-[var(--herr-ink)] hover:bg-[#FFFFFF]'
              }`}
            >
              # {s.name}
              {s.min_tier !== 'collective' && (
                <span className="ml-1 text-[0.7rem] text-[var(--herr-magenta)] uppercase">{s.min_tier}</span>
              )}
            </button>
          ))}
        </div>

        <div className="p-4 border-t border-[rgba(26,15,26,0.08)]">
          <button
        ...
        <div className="p-4 border-t border-[rgba(26,15,26,0.08)]">
          <div className="bg-[#1a0a12] border border-[var(--herr-magenta)]/30 p-3 text-[0.75rem] text-[rgba(26,15,26,0.6)] leading-relaxed">
            <p className="text-[var(--herr-magenta)] font-medium mb-1">Crisis?</p>
            <p>Call or text <strong className="text-[var(--herr-ink)]">988</strong></p>
          </div>
        </div>
      </aside>
      ...
      <button
        ...
        className="md:hidden fixed bottom-4 left-4 z-20 bg-[#FFFFFF] border border-[rgba(26,15,26,0.08)] px-3 py-2 text-[0.8rem] text-[rgba(26,15,26,0.6)]"
      >
        {sidebarOpen ? 'Close' : 'Spaces'}
      </button>

      {/* ---- Main content ---- */}
      <div className="flex-1 flex flex-col overflow-hidden bg-[#FFFFFF]/30">
        {view === 'spaces' ? (
          /* ========== SPACES VIEW ========== */
          activeThread ? (
            /* ---- Thread / Posts view ---- */
            <>
              <header className="p-4 border-b border-[rgba(26,15,26,0.08)] flex items-center gap-3">
                <button
                  onClick={() => setActiveThread(null)}
                  className="text-[rgba(26,15,26,0.6)] hover:text-[var(--herr-ink)] text-[0.85rem]"
                >
                  &larr; Back
                </button>
                <div className="flex-1 min-w-0">
                  <h3 className="text-[var(--herr-ink)] font-medium text-[0.95rem] truncate">
                    {activeThread.pinned && <span className="text-[var(--herr-magenta)] mr-1">Pinned</span>}
                    {activeThread.title}
                  </h3>
                  <p className="text-[0.75rem] text-[rgba(26,15,26,0.4)]">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {/* Thread body */}
                {activeThread.body && (
                  <div className="bg-[var(--herr-cream)] border border-[rgba(26,15,26,0.08)] p-4 text-[0.88rem] text-[rgba(26,15,26,0.6)] leading-relaxed">
                    {activeThread.body}
                  </div>
                )}
                  ...
                  return (
                    <div key={p.id} className="group">
                      <div className="flex items-start gap-3">
                        <div className="w-8 h-8 rounded-full bg-[rgba(26,15,26,0.08)] flex items-center justify-center text-[0.7rem] text-[rgba(26,15,26,0.6)] shrink-0">
                          {authorName(p.profiles).charAt(0).toUpperCase()}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-baseline gap-2">
                            <span className="text-[var(--herr-ink)] text-[0.85rem] font-medium">
                              {authorName(p.profiles)}
                            </span>
                            <span className="text-[0.7rem] text-[rgba(26,15,26,0.4)]">
                              {timeAgo(p.created_at)}
                            </span>
                          </div>
                          <p className="text-[0.88rem] text-[rgba(26,15,26,0.6)] leading-relaxed mt-1 whitespace-pre-wrap">
                            {p.body}
                          </p>
                          {/* Actions */}
                          <div className="flex items-center gap-3 mt-2">
                            <button
                              ...
                              className={`text-[0.75rem] flex items-center gap-1 transition-colors ${
                                userClapped ? 'text-[var(--herr-magenta)]' : 'text-[rgba(26,15,26,0.4)] hover:text-[var(--herr-ink)]'
                              }`}
                            >
                            ...
                            {!isOwn && (
                              <>
                                <button
                                  ...
                                  className="text-[0.7rem] text-[rgba(26,15,26,0.4)] hover:text-[var(--herr-magenta)] opacity-0 group-hover:opacity-100 transition-opacity"
                                >
                                  Flag
                                </button>
                                <button
              ...
              <div className="p-4 border-t border-[rgba(26,15,26,0.08)]">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-[var(--herr-cream)] border border-[rgba(26,15,26,0.08)] px-4 py-2 text-[0.88rem] text-[var(--herr-ink)] placeholder:text-[rgba(26,15,26,0.4)] focus:outline-none focus:border-[var(--herr-magenta)]"
                  />
                  <button
            ...
            /* ---- Thread list view ---- */
            <>
              <header className="p-4 border-b border-[rgba(26,15,26,0.08)] flex items-center justify-between">
                <div>
                  <h3 className="text-[var(--herr-ink)] font-medium text-[0.95rem]">
              ...
              <div className="flex-1 overflow-y-auto">
                {/* New thread form */}
                {showNewThread && (
                  <div className="p-4 border-b border-[rgba(26,15,26,0.08)] bg-[var(--herr-cream)]">
                    <input
                      value={newThreadTitle}
                      onChange={e => setNewThreadTitle(e.target.value)}
                      placeholder="Thread title"
                      className="w-full bg-transparent border border-[rgba(26,15,26,0.08)] px-4 py-2 text-[0.88rem] text-[var(--herr-ink)] placeholder:text-[rgba(26,15,26,0.4)] focus:outline-none focus:border-[var(--herr-magenta)] mb-2"
                    />
                    <textarea
                      value={newThreadBody}
                      ...
                      rows={3}
                      className="w-full bg-transparent border border-[rgba(26,15,26,0.08)] px-4 py-2 text-[0.88rem] text-[rgba(26,15,26,0.6)] placeholder:text-[rgba(26,15,26,0.4)] focus:outline-none focus:border-[var(--herr-magenta)] mb-2 resize-none"
                    />
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setShowNewThread(false)} className="text-[0.78rem] text-[rgba(26,15,26,0.6)] px-3 py-1.5">
                        Cancel
                      </button>
                    </div>
                  </div>
                ...
                {threads.length === 0 ? (
                  <div className="p-8 text-center">
                    ...
                    <p style={{ fontSize: '1rem', color: 'var(--herr-ink)', fontWeight: 600, marginBottom: 8 }}>
                  ...
                  threads.map(t => (
                    <button
                      ...
                      className="w-full text-left p-4 border-b border-[rgba(26,15,26,0.08)] hover:bg-[#FFFFFF] transition-colors"
                    >
                      <div className="flex items-start gap-3">
                        <div className="flex-1 min-w-0">
                          <p className="text-[var(--herr-ink)] text-[0.88rem] font-medium truncate">
                            {t.pinned && <span className="text-[var(--herr-magenta)] mr-1 text-[0.75rem]">[Pinned]</span>}
                            {t.title}
                          </p>
                          <p className="text-[0.75rem] text-[rgba(26,15,26,0.4)] mt-1">
          ...
          dmPartner ? (
            <>
              <header className="p-4 border-b border-[rgba(26,15,26,0.08)] flex items-center gap-3">
                ...
                <p className="text-[var(--herr-ink)] text-[0.9rem] font-medium">
                  ...
                  return (
                    <div key={dm.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[75%] px-4 py-2 text-[0.88rem] leading-relaxed ${
                        isOwn
                          ? 'bg-[var(--herr-magenta)]/20 text-[var(--herr-ink)] border border-[var(--herr-magenta)]/30'
                          : 'bg-[var(--herr-cream)] text-[rgba(26,15,26,0.6)] border border-[rgba(26,15,26,0.08)]'
                      }`}>
                        <p className="whitespace-pre-wrap">{dm.body}</p>
                        <p className="text-[0.65rem] text-[rgba(26,15,26,0.4)] mt-1">{timeAgo(dm.created_at)}</p>
                      </div>
                    </div>
                  );
                })}
            ...
            <>
              <header className="p-4 border-b border-[rgba(26,15,26,0.08)]">
                <h3 className="text-[var(--herr-ink)] font-medium text-[0.95rem]">Direct Messages</h3>
              </header>

              <div className="flex-1 overflow-y-auto">
                {/* New DM */}
                <div className="p-4 border-b border-[rgba(26,15,26,0.08)]">
                  <div className="flex gap-2">
                    <input
                      value={newDmPartnerId}
                      onChange={e => setNewDmPartnerId(e.target.value)}
                      placeholder="Enter member ID to message..."
                      className="flex-1 bg-[var(--herr-cream)] border border-[rgba(26,15,26,0.08)] px-3 py-2 text-[0.82rem] text-[var(--herr-ink)] placeholder:text-[rgba(26,15,26,0.4)] focus:outline-none focus:border-[var(--herr-magenta)]"
                    />
                ...
                {dmList.length === 0 ? (
                  <div className="p-8 text-center">
                    <p style={{ fontSize: '1rem', color: 'var(--herr-ink)', fontWeight: 500, marginBottom: 6 }}>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="min-w-0 flex-1">
                          <p className="text-[var(--herr-ink)] text-[0.85rem] truncate">
                            {c.partnerId.slice(0, 8)}...
                          </p>
                          <p className="text-[0.78rem] text-[rgba(26,15,26,0.4)] truncate mt-0.5">{c.body}</p>
                        </div>
                        <div className="flex items-center gap-2 shrink-0 ml-3">
                          <span className="text-[0.7rem] text-[rgba(26,15,26,0.4)]">{timeAgo(c.created_at)}</span>
                          {!c.read && <span className="w-2 h-2 rounded-full bg-[var(--herr-magenta)]" />}
                        </div>
                      </div>
                    </button>
                  ))
