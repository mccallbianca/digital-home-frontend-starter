...
export default function DashboardClient({
  ...
  // Dashboard nav now lives in MemberNav (src/components/layout/MemberNav.tsx).
  // Old in-component navItems removed along with the duplicate sidebar.
        ...
        <main style={{ background: 'var(--herr-cream)', padding: 'clamp(24px, 4vw, 32px)', minHeight: '100vh' }}>
          ...
          {plan && (
            <p
              style={{
                ...
                letterSpacing: '2px',
                color: plan === 'elite' ? '#2563EB' : plan === 'personalized' ? 'var(--herr-magenta)' : '#666666',
                marginBottom: 32,
              }}
            >
              {tierLabel}
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              ...
              <span style={{ fontSize: 13, color: 'rgba(26,15,26,0.4)' }}>3:24</span>
            </div>
          </div>
          ...
          <div className="cards-grid">
            {cards.map((card) => (
              ...
              >
                ...
                {card.status && (
                  <p style={{ fontSize: 12, color: 'var(--herr-magenta)', marginTop: 'auto' }}>{card.status}</p>
                )}
                {card.locked && (
