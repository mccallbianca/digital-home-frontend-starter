  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--herr-cream)', padding: '80px 24px 60px' }}>
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        ...
        <p style={{ fontSize: 16, color: 'rgba(26,15,26,0.6)', textAlign: 'center', marginBottom: 40 }}>
        ...
        {sessions.length > 0 ? sessions.map((session) => {
          ...
          return (
            <div
              ...
              style={{
                ...
                border: '1px solid rgba(26,15,26,0.08)', marginBottom: 16,
              }}
            >
              ...
              <p style={{ fontSize: 14, color: 'rgba(26,15,26,0.7)', lineHeight: 1.6, marginBottom: 16 }}>
                {session.description}
              </p>
              ...
              <div style={{ width: '100%', height: 4, background: 'rgba(26,15,26,0.08)', borderRadius: 2, marginBottom: 16, overflow: 'hidden' }}>
              ...
              {isRegistered ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 13, color: 'rgba(26,15,26,0.4)', marginLeft: 8 }}>Calendar invite sent to {userEmail}</span>
                </div>
              ) : isFull ? (
                <button
                  disabled
                  style={{
                    ...
                    color: 'rgba(26,15,26,0.5)', borderRadius: 12, border: 'none',
          ...
          <div style={{ background: '#FFFFFF', borderRadius: 16, padding: 32, border: '1px solid rgba(26,15,26,0.08)', textAlign: 'center' }}>
            <p style={{ fontSize: 15, color: 'rgba(26,15,26,0.5)' }}>No upcoming sessions scheduled yet. Check back soon.</p>
          </div>
        )}

        {/* Past sessions */}
        {pastSessions.length > 0 && (
          <div style={{ marginTop: 40, borderTop: '1px solid rgba(26,15,26,0.08)', paddingTop: 32 }}>
            <p style={{ fontSize: 12, textTransform: 'uppercase', letterSpacing: '2px', color: 'rgba(26,15,26,0.4)', marginBottom: 16 }}>
              PAST SESSIONS
            </p>
            {pastSessions.map((s) => (
              <div key={s.id} style={{ background: '#FFFFFF', borderRadius: 12, padding: '16px 24px', border: '1px solid rgba(26,15,26,0.08)', marginBottom: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <p style={{ fontSize: 14, color: '#FFFFFF' }}>{s.title}</p>
                  <p style={{ fontSize: 13, color: 'rgba(26,15,26,0.4)' }}>
                    {new Date(s.scheduled_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                  </p>
                </div>
                <span style={{ fontSize: 12, color: 'rgba(26,15,26,0.3)' }}>Recordings coming soon</span>
              </div>
            ))}
          </div>
        )}
