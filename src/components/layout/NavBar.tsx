    ...
    <>
      ...
      >
        <div className="max-w-[1200px] mx-auto flex items-center justify-between h-[72px]">
          ...
          <Link
            href="/"
            className="font-display text-xl font-light tracking-[0.25em] uppercase text-[var(--herr-white)] hover:text-[var(--herr-magenta)] transition-colors duration-300"
          >
            HERR™
          </Link>
          ...
          <div className="flex flex-col gap-6 mt-4">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="font-display text-4xl font-light text-[var(--herr-white)] hover:text-[var(--herr-magenta)] transition-colors"
              >
                {link.label}
              </Link>
            ))}
